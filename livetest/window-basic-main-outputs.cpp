...
inline BasicOutputHandler::BasicOutputHandler(MainWindow *main_) : main(main_)
{
}
...
SimpleOutput::SimpleOutput(MainWindow *main_) : BasicOutputHandler(main_)
{
...
{
	//const char *path =
	//	"D:\\code\\obs-studio\\build32\\rundir\\Debug\\bin\\32bit\\";
	//const char *format =
	...
	obs_data_set_string(settings, "path",
		"D:\\code\\obs-studio\\build32\\rundir\\Debug\\bin\\32bit\\test.mp4");
...
bool SimpleOutput::StartRecording()
{
	...
	if (!obs_output_start(fileOutput)) {
		...
		if (error)
			...
			error_reason = "Output.StartFailedGeneric";
...
bool SimpleOutput::StartReplayBuffer()
{
	...
	if (!obs_output_start(replayBuffer)) {
		QMessageBox::critical(main, "Output.StartReplayFailed",
...
//
//	AdvancedOutput(MainWindow *main_);
//
...
//
//AdvancedOutput::AdvancedOutput(MainWindow *main_) : BasicOutputHandler(main_)
//{
//	const char *recType =
					 ...
					 const char *format, bool ffmpeg)
{
	...
	string dst = "D:\\code\\obs-studio\\build32\\rundir\\Debug\\bin\\32bit\\test.mp4";
	return dst;
}
